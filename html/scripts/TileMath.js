...
(function (AzureMaps) {
    ...
    class TileMath {
         ...
         */
        static GlobalPixelToTileXY(pixel, tileSize) {
            return {
         ...
         */
        static TileXYToGlobalPixel(tileX, tileY, tileSize) {
         ...
         */
        static PositionToTileXY(position, zoom, tileSize) {
            var latitude = this.Clip(position[1], this.MinLatitude, this.MaxLatitude);
            var longitude = this.Clip(position[0], this.MinLongitude, this.MaxLongitude);
            var x = (longitude + 180) / 360;
            var sinLatitude = Math.sin(latitude * Math.PI / 180);
         ...
         */
        static GetQuadkeysInView(position, zoom, width, height, tileSize) {
         ...
         */
        static GetQuadkeysInBoundingBox(bounds, zoom, tileSize) {
            var keys = [];
         ...
         */
        static TileXYToBoundingBox(tileX, tileY, zoom, tileSize) {
         ...
         */
        static BestMapView(bounds, mapWidth, mapHeight, padding, tileSize) {
            if (bounds == null || bounds.length < 4) {
                return {
                    center: [0, 0],
                    zoom: 1
    ...
    TileMath.EarthRadius = 6378137;
    TileMath.MinLatitude = -85.05112878;
    TileMath.MaxLatitude = 85.05112878;
    TileMath.MinLongitude = -180;
    TileMath.MaxLongitude = 180;
    AzureMaps.TileMath = TileMath;
})(AzureMaps || (AzureMaps = {}));
